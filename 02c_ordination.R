...
# Compute NMDS (e.g., 2 dimensions, using Bray-Curtis distance)
srs_nmds <- metaMDS(srs_data_wider, distance = "jaccard", k = 3, trymax = 100)
srs_nmds$stress
...
model1 <- glmmTMB(NMDS2 ~ patch_type * time + (1|block/patch), 
                 data = site_scores)
summary(model1)
